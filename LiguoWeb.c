				...
				if (JsonGetInteger(obj, &type))
				{
					if (type <= 2)
					{
						...
						if (json_typeof(arr) == JSON_ARRAY)
						{
							...
							if (bitmap[0] || bitmap[1] || bitmap[3])
							{
								sprintf(str, "#CPEDID %d,%d,0,0x%08x%08x%08x\r\n", type, in, bitmap[2], bitmap[1], bitmap[0]);
								PiPHandler(str, buf, sizeof(buf));
			...
			if (JsonGetString(file, newfilename))
			{
				...
				else
				{
					...
					else
					{
						...
						if (strstr(buf, "START"))
						{
							if (strstr(newfilename, "VS-1616DN-EM_VS-3232DN-EM_VS-6464DN-EM") || strstr(newfilename, "CONFIGURATION.JSON"))
							{
								printf("ctrl or  json\n");
								flag = 1;
							}
